...
def car_control(picar, servo_d=0, speed=0):
    picar.set_dir_servo_angle(servo_d)
    picar.forward(speed)


def camera_control(picar, servo_h=0, servo_v=-20):
    picar.set_camera_servo1_angle(servo_h)
    picar.set_camera_servo2_angle(servo_v)
...
def drive(video_file):
    ...
    camera_control(picar_control, 0, -20)
    ...
    try:
        while cap.isOpened():
            ...
            if len(lanes_in_roi) == 0:
                car.speed = 0
                # stop car
                car_control(picar_control, 0, 0)
            else:
                car.speed = 20
                car_control(picar_control, car.current_steering_angle, car.speed)
            ...
            if cv2.waitKey(1) & 0xFF == ord('q'):
                car_control(picar_control, 0, 0)
                camera_control(picar_control, 0, 0)
                break
...
def average_slope_intercept(line_segments):
    ...
    if len(left_fit) > 0:
        if len(left_fit) > 4:
            left_fit.sort(key=itemgetter(0))
            left_fit = left_fit[2:-2]
        left_fit_average = np.average(left_fit, axis=0)
        lane_lines.append(make_points(left_fit_average))
    if len(right_fit) > 0:
        if len(right_fit) > 4:
            right_fit.sort(key=itemgetter(0))
            right_fit = right_fit[2:-2]
...
def display_lines(frame, lines, line_color=(255, 255, 255), line_width=5):
...
def test_photo(file):
    ...
    lane_lines_img = display_lines(image_blur, lanes_in_roi)
    ...
    cv2.imwrite('Heading Lanes.jpg', heading_line_img)
...
if __name__ == '__main__':
    car = PiCar('PiCar')
    # picar_control = Picarx()
    # drive('filename.avi')
    test_video('filename.avi')
    # test_photo('test_pic/image_090_090.png')
